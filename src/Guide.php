...
final class Guide
{
    public static function text(): string
    {
        ...
        A back-office CRM: **contacts** (people) and the **organizations** (companies)
        they belong to; activities and a deal pipeline arrive in later slices. It is
        **PII**, so every tool is gated by the `nimbuscms.crm` capability: a read needs
        `nimbuscms.crm:read`, a write needs `nimbuscms.crm:write`. A content `*:write`
        token cannot reach it, and a tool you lack the capability for is invisible.
        ...
        `org_id`. Values are stored as you send them and escaped when displayed; there is
        no public page for CRM data.
        MD;
    }
}
